  ...
  <div class="nav-menu-wrapper">
    <ul class="menu-list">
      <div v-for="(item, index) in menuList" :key="index" class="menu-item">
        <li
          :class="{ activeNav: index == current, hiddenStatus: hiddenStatus }"
        ...
        >
          <span
            ...
            v-show="!hiddenStatus"
          ></span>
          <span class="item-title" :class="{ hiddenStatus: hiddenStatus }">{{
            item.title
          }}</span>
          <el-tooltip
            class="tooltip"
...
const menuList = [
  ...
  {
    icon: "icon-chat-bolt",
    routePath: "/sessionChat",
    title: "万能问答",
    index: 0,
  },
  {
    icon: "icon-tiaoseban ",
    routePath: "/sessionDraw",
    title: "AI画图",
    index: 1,
  },
  {
    icon: "icon-qixiangleida",
    routePath: "/sessionDomain",
    title: "场景对话",
    index: 2,
  },
  {
...
export default {
  ...
  methods: {
    activeIndex() {
      ...
      this.current =
        filter == null || filter.length === 0 ? -1 : filter[0].index;
    },
    changeMenu(item) {
